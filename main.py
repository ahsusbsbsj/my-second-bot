...
stopuser = {}
token = "8093075410:AAFAOMddUkIem0Zh5WxKTF_8rTv8-0aUd0k" #توكنك هنا
bot=telebot.TeleBot(token,parse_mode="HTML")
admin=5617279645 #ايدي الادمن بدون علامات تنصيص
command_usage = {}
...
''',reply_markup=keyboard)
		...
		''',reply_markup=keyboard)
			...
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text=f"Stripe Charge $1",callback_data='br')
		keyboard.add(contact_button)
...
• @D9_M7 : @D9_M7</b>'''
					...
					if "Charge" in last or 'Insufficient funds' in last or 'Invalid postal code or street address' in last or 'OTP' in last or 'Insufficient Funds' in last:
						live += 1
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='Nada-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
